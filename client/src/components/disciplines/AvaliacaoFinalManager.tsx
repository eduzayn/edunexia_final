...
export function AvaliacaoFinalManager() {
  ...
  useEffect(() => {
    ...
    async function carregarQuestoes() {
      try {
        ...
        console.error("Erro ao carregar questões da avaliação final:", error);
        toast({
          title: "Erro",
          description: "Não foi possível carregar as questões da avaliação final",
          variant: "destructive"
        });
      } finally {
        setIsLoading(false);
  ...
  async function handleSalvarQuestao() {
    ...
    try {
      ...
      if (editandoQuestaoId) {
        ...
        response = await disciplinasService.atualizarQuestaoAvaliacaoFinal(
          disciplinaId, 
          editandoQuestaoId, 
          questaoData
        );
        ...
        // Nova questão
        response = await disciplinasService.adicionarQuestaoAvaliacaoFinal(
          disciplinaId, 
          questaoData
        );
  ...
  async function handleRemoverQuestao(questaoId: string) {
    try {
      setIsLoading(true);
      await disciplinasService.removerQuestaoAvaliacaoFinal(disciplinaId, questaoId);
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={isLoading || questoes.length >= 10}
          >
